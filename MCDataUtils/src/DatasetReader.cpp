...
using namespace cv;
using namespace std;
// Configuration and data reading functions

void DatasetReader::initialize(MCDataUtilSettings refocus_set) {
    ...
    current_index = 0;
    VLOG(0) << "Reading from the calibration file";
...
void DatasetReader::read_kalibr_data(string path) {
    ...
    VLOG(1) << "Reading calibration (kalibr) data from " << path << "...";
...
void DatasetReader::read_imgs(string path) {
    ...
    VLOG(1)<<"Reading images to refocus...";

    // VLOG(2)<<"UNDISTORT_IMAGES flag is "<<UNDISTORT_IMAGES;
    // VLOG(2)<<"RADTAN_FLAG flag is "<<RADTAN_FLAG;
    // VLOG(2)<<"ASYNC_FLAG flag is "<<ASYNC_FLAG;
    ...
    for (int i=0; i<num_cams_; i++) {
        ...
        // VLOG(2)<<"Camera "<<i+1<<" of "<<num_cams_<<"..."<<endl;
    ...
    if(ALL_FRAME_FLAG) {
        ...
        if (ASYNC_FLAG) {
            ...
            if (end>all_img_names[0].size()) {
                //VLOG(0)<<"End frame is greater than number of frames!" <<endl;
                end = all_img_names[0].size();
            }
        }
    }
    ...
    for (int i = 0; i < num_cams_; i++) {
        ...
        for (int j = begin; j < end; j += skip + 1) {
            ...
            if (i == 0) {
                ...
                // minor change to add kitti timestamps
                // change to substr(0,19) for the others 
                long int tstamp_ref = atol(filename.substr(13, 19).c_str());
                double stmp = tstamp_ref;
                // double stmp = tstamp_ref * 1e-9;
                VLOG(0) << "Timestamp: " << setprecision(9) << std::fixed << stmp << endl;
    ...
    VLOG(1)<<"Done reading all Images...";
}
...
void DatasetReader::read_binary_imgs(string path) {
    ...
    if(!imgs_read_) {
        ...
        // VLOG(0)<<"READING IMAGES TO REFOCUS...";
...
void DatasetReader::loadNext(vector<cv::Mat>& imgset) {
    ...
    for (int cam_ind=0; cam_ind< num_cams_; cam_ind++){
        ...
        //cv::resize(imgg, imgg, calib_img_size_);
        imgg /= 255.0;
...
void DatasetReader::getNext(vector<cv::Mat>& imgs, double& timeStamp, std::deque<sensor_msgs::Imu>& imu_msgs){}



void DatasetReader::getNext(vector<cv::Mat>& imgs, double& timeStamp, std::deque<sensor_msgs::Imu>& imu_msgs, std::deque<sensor_msgs::NavSatFix>& gps_msgs){}