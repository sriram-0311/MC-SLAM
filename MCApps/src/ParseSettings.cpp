...
void programOptions(po::variables_map& vm, const string& filename, bool h){
...
//            ("skip", po::value<int>(), "Successive frames to skip (used if all_frames is set to 0)")
            ...
            ("imu", po::value<int>()->default_value(0), "1 if imu is used and 0 if imu is not used")
            ("ImuMapFrame", po::value<int>()->default_value(0), "1 if mapped navigation frame is NED and 0 if mapped navigation frame is ENU")
            ("gps", po::value<int>()->default_value(0), "1 if gps is used and 0 if gps is not used")

            ("rosbag_path", po::value<string>()->default_value(""), "path where rosbag data is located")
            ("parse_bag", po::value<int>()->default_value(0), "0 if rosdata is used online or else 1 if its used offline by parsing it first")
            ("skip_frames", po::value<int>()->default_value(0), "if skip is 3, it reads every 3rd frame")

            ("graph_logs", po::value<string>()->default_value(""), "Stores the path to the graph log ")
            ("relocalization", po::value<int>()->default_value(0), "Relocalization mode")
            ("CustomVocabFile", po::value<string>()->default_value(""), "Stores the custom voc file path")
            ("DataBase", po::value<string>()->default_value(""), "Stores the database of the prev run ")
            ("GraphLogs", po::value<string>()->default_value(""), "Stores the path to the graph log ")
            ("MapLogs", po::value<string>()->default_value(""), "Stores the landmark descriptor correspondences of the prev run ")
            ("navability", po::value<int>()->default_value(0), "Set 1 if parsing navability maps")
            ;
...
void parse_settings(const string& filename, MCDataUtilSettings &settings, bool h) {
    ...
    parseMCDataUtilParams(settings, vm);
...
void parseMCDataUtilParams(MCDataUtilSettings &settings, const po::variables_map& vm ){
    ...
    settings.imu = vm["imu"].as<int>();
    settings.gps = vm["gps"].as<int>();
    settings.rosbag_path = vm["rosbag_path"].as<string>();
    settings.parse_bag = vm["parse_bag"].as<int>();
    settings.skip_frames = vm["skip_frames"].as<int>();
    settings.relocalization = vm["relocalization"].as<int>();
    settings.customVocabFile = vm["CustomVocabFile"].as<string>();
    settings.dataBase = vm["DataBase"].as<string>();
    settings.graphLogs = vm["GraphLogs"].as<string>();
    settings.mapLogs = vm["MapLogs"].as<string>();
    settings.navability = vm["navability"].as<int>();
    ...
    settings.graphLogs = vm["graph_logs"].as<string>();
