...
static cv::Mat convertPose3_GTSAM2CV(gtsam::Pose3 &pose){
    cv::Mat poseMat = cv::Mat::eye(4,4, CV_64F);
    poseMat.at<double>(0,0) = pose.rotation().matrix()(0,0);
    poseMat.at<double>(0,1) = pose.rotation().matrix()(0,1);
    poseMat.at<double>(0,2) = pose.rotation().matrix()(0,2);
    poseMat.at<double>(1,0) = pose.rotation().matrix()(1,0);
    poseMat.at<double>(1,1) = pose.rotation().matrix()(1,1);
    poseMat.at<double>(1,2) = pose.rotation().matrix()(1,2);
    poseMat.at<double>(2,0) = pose.rotation().matrix()(2,0);
    poseMat.at<double>(2,1) = pose.rotation().matrix()(2,1);
    poseMat.at<double>(2,2) = pose.rotation().matrix()(2,2);
    
    poseMat.at<double>(0,3) = pose.translation().x();
    poseMat.at<double>(1,3) = pose.translation().y();
    poseMat.at<double>(2,3) = pose.translation().z();
