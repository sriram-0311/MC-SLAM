...
BOOST_CLASS_EXPORT(PreintegratedCombinedMeasurements);
...
public:
    ...
    struct LooperVar
    {
        gtsam::Pose3 rel_pose_;
        int best_match_idx_;
        int curr_frame_idx_;
        bool loop_candidate_ = false;
    };
    LooperVar lvar;
    ...
    void addKeyframeGPS(bool addIMUFactor);
    ...
    void optimizePose();
    double euclidean(tuple<double, double, double> gps_msg1, tuple<double, double, double> gps_msg2);
    void change_vision_kf_info();

    // Smart factor based functions
    bool SmartFactor_backend(bool loop_closed);
    bool RelocalizationGraphConstructor(gtsam::NavState currentState);
    void addIMUFactorRelocalization(int prev_kf, int curr_kf, gtsam::NavState currentState);
    bool GlobalRelocalizedPoseNode();
    void UpdateVariablesRelocalization();
    void insertSmartFactorinGraph(int lid, vector<int> compcam_ids, IntraMatch* im, MultiCameraFrame*  currentFrame);
    void insert_priors_smartFactor();
    bool insert_prior_condition();
    void process_SmartFactor(MultiCameraFrame* currentFrame, GlobalMap* map);
    void addPosetoInitialEstimate(MultiCameraFrame* currentFrame);
    void UpdateVariables_SmartFactors();
    void Smartfactor_deleteLandmark(int lid, GlobalMap* map);
    void addLoopClosure(int curr_frame_id);
    ...
    // smart factor variables
    boost::shared_ptr<gtsam::CameraSet<gtsam::PinholePose<gtsam::Cal3_S2>>> cameraRig;
    // Smart factor params
    gtsam::SmartProjectionParams smartFactor_params;
    // A map of lids and rigfactor pointers
    std::map<int, RigFactor::shared_ptr> smartFactor;

    std::map<int, FactorIndex> lm_to_facInds;
    FastMap<FactorIndex, KeySet> factorNewAffectedKeys;
    std::map<FactorIndex, int> newFactor_to_lm;

    // to debug
    std::vector<int> debug_lids;
    std::map<int, int> deleted_landmarks;
    ...
    int prev_Kf = -1;
    int prevstate = -1;
    void addIMUFactor(int prev_kf, int curr_kf);
    void addBetweenFactor_Poses(int prev_kf, int curr_kf);
    void addPoseInitialEstimateRelocalization(MultiCameraFrame* currentFrame);

    
    int total_isam_errors = 0;
    int total_succ_isam = 0;
    int total_prior_isam_calls = 0;

    // gps
    tuple<double, double, double> processGPS_NED(sensor_msgs::NavSatFix gps_msg);
    tuple<double, double, double> processGPS_ENU(sensor_msgs::NavSatFix gps_msg);
    void addGPSFactor(std::deque<sensor_msgs::NavSatFix> &gps_msgs_combined, int current_pose_id );
    gtsam::Pose3 E_T_V;
    gtsam::Pose3 Tbg;
    bool added_E_T_V_init_est = false;
    bool use_E_T_V_init_est = false;
    int gps_id = -1;
    bool debug_prior = true;

    tuple<double, double, double> last_gps_msg = make_tuple(0,0,0);

    //debug 
    int gps_factor_count = 0;
    gtsam::FactorIndices toBeRemovedFactorIndices_test;
