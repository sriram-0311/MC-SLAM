...
MultiCameraFrame::MultiCameraFrame(vector<Mat> img_set, vector<Mat> segmap_set,DBoW2::ORBVocabulary* vocab, ORBextractor* orb_obj, vector<ORBextractor*> orbs,
...
MultiCameraFrame::MultiCameraFrame(bool dummykf, double tStamp, CamArrayConfig& camconfig): dummy_kf(dummykf), timeStamp(tStamp), camconfig_(camconfig) {

    // as of now, camconfig is a reference variable in the class, so it being expected here. we could change to pointers.
    orBextractors = {};
    numTrackedLMs = 0;
    intramatch_size = 0;
    mono_size = 0;
    num_inliers_per_view = {};
    num_triangulated_per_view = {};
    rotations_ptr = {};
    translations = {};
    pose = Mat::eye(4, 4, CV_64F);
    cov = Mat::zeros(6, 6, CV_64F);
    orBextractor = nullptr;
    orb_vocabulary = nullptr;
    imgs.clear();
    segMasks.clear();
...
void MultiCameraFrame::extractFeaturesParallel(){
    ...
    VLOG(1) <<"undistorted all images" <<endl;
    for(int cam_ind = 0; cam_ind < num_cams_ ; cam_ind++)
        ths[cam_ind].join();
}
...
void MultiCameraFrame::UndistortKeyPoints(int cam) {
    ...
    cv::Mat mat_undist(N, 1, CV_32F);

       cv::Mat K_mat(camconfig_.K_mats_[cam].rows, camconfig_.K_mats_[cam].cols, CV_32F);
    cv::Mat dist_coeffs(camconfig_.dist_coeffs_[cam].rows, camconfig_.dist_coeffs_[cam].cols, CV_32F);

    camconfig_.K_mats_[cam].convertTo(K_mat, CV_32F);
    camconfig_.dist_coeffs_[cam].convertTo(dist_coeffs, CV_32F);
    ...
    // Undistort points
    mat = mat.reshape(2);
    cv::undistortPoints(mat, mat, K_mat, dist_coeffs, cv::Mat(), K_mat);
    mat = mat.reshape(1);
    ...
    for (int i = 0; i < N; i++) {
        ...
        image_kps_undist[cam].push_back(kp);
...
void MultiCameraFrame::computeIntraMatches(vector<IntraMatch>& matches,vector<DBoW2::NodeId >& words_){
    ...
    auto* featVecs = new DBoW2::FeatureVector[num_cams_];
    auto* featVecIters = new  DBoW2::FeatureVector::const_iterator[num_cams_];
    auto* featVecEnds = new DBoW2::FeatureVector::const_iterator[num_cams_];
